						...
						<div id="ModalEdit<?php echo $key['id_pkl'];?>" role="dialog" class="modal fade">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="<?php echo site_url(); ?>/writer/editnews/<?php echo $key['id_pkl']; ?>" enctype="multipart/form-data" method="post">
									<div class="modal-body">
										...
										<input type="number" min="0" step="1" class="form-control ins <?php echo $key['id_pkl'] . "_jumlah"; ?>" placeholder="Jumlah PKL" value="<?php echo $key['jml_pkl'];?>" id="jml_pkl" name="jml_pkl"><br>
										<input type="file" class="form-control ins" id="srt_pengantar" name="srt_pengantar"><br>
									...
									<div class="modal-footer">
										<button class="width-15 pull-left btn btn-warning ">
											<a href="<?php echo site_url(); ?>/writer/index" style="color: white;">cancel</a>
										</button>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				$('.tgl_selesai').on('change', (function() {
					...
					var nilai_divisi = $('.' + divisi).val();

					var mulai = $(this).data('mulai');
					var nilai_mulai = $('.' + mulai).val();

					var nilai_selesai = $(this).vall();

					var jumlah = $(this).data('jumlah');

					var sisa_peserta = $(this).data('sisa_peserta');
					...
					$.ajax({
						method: 'GET',
						url: "<?php echo site_url('writer/hitung_sisa/'); ?>" + nilai_divisi + "/" + nilai_mulai + "/" + nilai_selesai,
						processData: false,
						success: function(response) {
							$('.' + jumlah).removeAttr('max');
							$('.' + jumlah).attr('max', response);
							$('.' + jumlah).val('');
							$('.'+ sisa_peserta).val(response);
						}
					})
				}))
			})
